...
async function run() {
    ...
    console.log('🔄 Syncing PKU1299...');
    await axios.post('https://sca.jne.id/lm-api/sync/delivery?refresh=true', { from: date, to: date, couriers: ['PKU1299'] }, { headers: { Authorization: token } });
    
    console.log('🔄 Fetching detail PKU1299...');
    const urlDetail = `https://sca.jne.id/lm-api/dashboard/report?&from=${date}&to=${date}&courier_id=PKU1299&result_type=list`;
    const res = await axios.get(urlDetail, { headers: { Authorization: token } });
    let resiList = res.data.data || [];
    ...
    console.time("⏳ Total Waktu AI (10 Paket)");
    let results = [];
    for(let r of toTest) {
        console.log(`\n📦 Resi: ${r.DRSHEET_CNOTE_NO} (${r.type})`);
        ...
        results.push({ resi: r.DRSHEET_CNOTE_NO, type: r.type, valid: hasil.valid, alasan: hasil.alasan, ms: timeMs });
    }
    console.timeEnd("⏳ Total Waktu AI (10 Paket)");
}
